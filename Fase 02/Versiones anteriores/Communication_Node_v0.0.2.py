...
class CommunicationNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('connection_serial', '/dev/ttyUSB0')  # Puerto serial por defecto
        self.declare_parameter('connection_mode', 'FULL')  # Modo de conexión (FULL o SAFE)
        self.declare_parameter('movement_frequency', 10.0)  # Frecuencia de publicación de movimiento
        self.declare_parameter('security_frequency', 5.0)   # Frecuencia de publicación de seguridad
        self.declare_parameter('power_frequency', 1.0)      # Frecuencia de publicación de energía
        self.declare_parameter('misc_frequency', 1.0)       # Frecuencia de publicación de misceláneos
    ...
    def initialize_connection(self):
        ...
        try:
            self.robot = Create2(serial_port)  # Conectar al Roomba
            self.robot.start()  # Iniciar el robot
            if mode == 'FULL':
                self.robot.oi_mode = MODES.FULL
            elif mode == 'SAFE':
                self.robot.oi_mode = MODES.SAFE
            self.get_logger().info(f'Conectado al Roomba en {serial_port} en modo {mode}')

            # Iniciar publicación de datos
            self.start_timers()

        except Exception as e:
            self.get_logger().error(f'Error al conectar con el Roomba: {e}')
    ...
    def publish_movement_data(self):
        if self.robot:
            data = {
                ...
                "left_encoder_counts": self.robot.left_encoder_counts,
                "right_encoder_counts": self.robot.right_encoder_counts,
            }
            msg = Movement()
    ...
    def publish_security_data(self):
        if self.robot:
            data = {
                ...
                "wheel_drop_left": self.robot.bumps_and_wheel_drops.wheel_drop_left,
                "wheel_drop_right": self.robot.bumps_and_wheel_drops.wheel_drop_right,
                "wall": self.robot.wall_sensor,
                "virtual_wall": self.robot.virtual_wall,
            }
            msg = Security()
    ...
    def publish_misc_data(self):
        if self.robot:
            data = {
                "cliff_left_signal": self.robot.cliff_left_signal,
                "cliff_front_left_signal": self.robot.cliff_front_left_signal,
                "cliff_front_right_signal": self.robot.cliff_front_right_signal,
                "cliff_right_signal": self.robot.cliff_right_signal,
                "wall_signal": self.robot.wall_signal,
            }
            msg = Misc()
    ...
    def publish_state_data(self):
        if self.robot:
            data = {
                "oi_mode": str(self.robot.oi_mode),
                "is_charging": self.robot.charging_state == 1,
                "battery_charge": self.robot.battery_charge,
            }
            msg = State()
    ...
    def connect_callback(self, request, response):
        try:
            if self.robot:
                self.robot.stop()
            self.robot = Create2(request.port)
            self.robot.start()
